...
export async function chatJSON({ messages, model = 'gpt-4.1-mini', maxTokens = 1400, temperature = 0.7, audio = null }) {
  ...
  // The audio-hearing model occasionally rejects strict JSON-mode outright
  // (a 400 "'messages' must contain the word json" even though it does) —
  // for audio calls we just ask for JSON in the prompt instead and lean on
  // the parsing fallback below; text-only calls keep the stricter, more
  // reliable enforced mode.
  if (!audio) body.response_format = { type: 'json_object' };
