...
export function ttsInstructions(tone, slow, direction) {
  const base =
    "You are a warm parent reading an English picture book aloud to your 4-year-old daughter, doing all the voices like a favorite storytime read-aloud — bright, animated, full of warmth and energy, never flat or hushed by default. Natural and unhurried, but never sleepy or subdued. Never sound like a narrator reading a script or an announcer. Give every sentence that ends in \"?\" a clear, unmistakable rising question intonation — even mid-passage, right after a flat statement — so it's obvious to a young child that it is a question, not a statement.";
  const byTone = {
