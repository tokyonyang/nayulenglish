...
async function cacheKeyFor(text, tone, slow) {
  const raw = `${text}|${tone || ''}|${slow ? 1 : 0}`;
...
async function generateAndPlay(text, tone, slow, cachePath) {
...
async function playOne(text, tone, slow, cacheable) {
  if (!text || !text.trim()) return;

  if (cacheable && supabaseReady) {
    try {
      const path = await cacheKeyFor(text, tone, slow);
      ...
      await generateAndPlay(text, tone, slow, path);
      return;
    } catch (e) {
      console.error('cached TTS failed, falling back', e);
    }
  }
  await generateAndPlay(text, tone, slow, null);
}
 ...
 *  (book read-aloud text) so it's synthesized once and reused. */
export async function speakLines(lines, { slow = false, muted = false, cacheable = false } = {}) {
  if (muted) return;
  cancelled = false;
  for (const line of lines) {
    if (cancelled) return;
    try {
      await playOne(line.text, line.tone, slow, cacheable);
    } catch (e) {
