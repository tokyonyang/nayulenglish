...
function normalizeSfxForSpeech(text) {
  return String(text || '')
    .replace(/~+/g, '') // an elongation mark for the eye, not a sound to voice
    .replace(/([a-zA-Z])\1{2,}/g, '$1$1') // "mmmmMM" -> "mmMM": still emphatic, no longer a run a TTS model chokes on
    .trim();
}

export function spreadLines(spread, characterVoices = {}, readingGuidance = '') {
  const lines = [];
  const wholeBookDirection = String(readingGuidance || '').trim();
  const pageDirection = String(spread.voiceDirection || '').trim();
  const direction = [
    wholeBookDirection ? `Parent's whole-book reading request: ${wholeBookDirection}` : '',
    pageDirection ? `This page's delivery: ${pageDirection}` : '',
  ].filter(Boolean).join(' ');
  ...
  for (const seg of segments) {
    ...
    lines.push({ text, tone: spread.tone || 'calm', direction, voice });
  }
  ...
  // Sound-effect text often has typographic flourish (tildes for an
  // elongated sound, a run of repeated letters like "mmmmMM") that's
  // meant to be READ as a design choice on the page, not spoken letter by
  // letter — a TTS model trying to literally voice a tilde or six m's in
  // a row can come out as odd, garbled noise. This only touches what gets
  // SPOKEN; the printed text on screen is never altered.
  (spread.sfx || []).forEach((s) => lines.push({ text: normalizeSfxForSpeech(s), tone: 'sfx', direction }));
  return lines;
}
...
export async function checkCacheStatus(spreads, { voice = '', characterVoices = {}, readingGuidance = '', slow = false, bookId = '' } = {}) {
  if (!bookId) return { cached: 0, total: 0 };
  const lines = (spreads || [])
    .flatMap((s) => spreadLines(s, characterVoices, readingGuidance))
...
export async function backupBookToDrive(book, { voice = '', characterVoices = {}, readingGuidance = '', slow = false, onProgress } = {}) {
  ...
  for (const line of (book.spreads || []).flatMap((s) => spreadLines(s, characterVoices, readingGuidance))) {
 ...
 *  isn't configured. */
export async function precacheSpreads(spreads, { voice = '', characterVoices = {}, readingGuidance = '', slow = false, bookId = '', onProgress } = {}) {
  if (!bookId) return;
  const lines = (spreads || []).flatMap((s) => spreadLines(s, characterVoices, readingGuidance));
...
export async function listenUntilSilence({
  ...
  silenceMs = 2400,
  // Requires this many ms of CONTINUOUS, unbroken loudness before counting
  // it as real speech — a brief hesitation sound ("음...") is usually
  // well under this, so it no longer starts the silence countdown (and
  // trigger an early send) on its own the way a single loud instant did.
  minSpeechMs = 550,
  maxMs = 25000,
  ...
  // 2048 (not 512) so the time-domain buffer is long enough for reliable
  // pitch detection below — frequency-domain loudness averaging still
  // works fine at this size, just with finer bins.
  ...
  let loudStreakStart = null; // when the CURRENT unbroken loud stretch began, or null if currently quiet
  let voicePitchDetected = false; // whether a genuine voice-like pitch has been seen during THIS loud streak
  ...
  await new Promise((resolve) => {
    function tick() {
      ...
      if (avg > THRESHOLD) {
        lastLoud = now;
        if (loudStreakStart == null) loudStreakStart = now;

        // A cough or a rustling sound can easily be loud AND sustained
        // enough to otherwise pass for speech — the thing loudness alone
        // can't tell apart from a real "네" or "yes" is pitch: genuine
        // voice has a clear, fairly stable fundamental frequency; noise
        // usually doesn't. Check periodically (not every frame — this is
        // the more expensive autocorrelation call) while a loud streak is
        // ongoing, and only count the streak as speech once real pitch
        // has actually been seen in it.
        if (now - lastPitchCheck > 150) {
          lastPitchCheck = now;
          analyser.getFloatTimeDomainData(timeData);
          const p = autocorrelatePitch(timeData, vadCtx.sampleRate);
          if (p != null && p > 60 && p < 800) voicePitchDetected = true; // plausible human-voice range only
        }
        if (voicePitchDetected && now - loudStreakStart > minSpeechMs) heardSpeech = true;
      } else {
        loudStreakStart = null; // the streak broke — a short "음" won't accumulate across gaps
        voicePitchDetected = false; // next streak needs its own fresh pitch evidence
      }
