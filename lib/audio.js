...
async function playOne(line, slow, cacheable, voice) {
  ...
  if (cacheable && supabaseReady) {
    try {
      const path = await cacheKeyFor(line, slow, voice);
      ...
      await generateAndPlay(line, slow, path, voice);
      return;
    } catch (e) {
      console.error('cached TTS failed, falling back', e);
    }
  }
  await generateAndPlay(line, slow, null, voice);
}
 ...
 *  short beats. The main line carries `voiceDirection` — a scene-specific
 *  delivery note written when the photo was scanned — so the reading
 *  reacts to what's actually happening on the page, not a generic mood. */
export function spreadLines(spread) {
  const lines = [];
  const text = normalizeForSpeech(spread.englishText);
  if (text) lines.push({ text, tone: spread.tone || 'calm', direction: spread.voiceDirection || '' });
 ...
 *  page text, asides, sound effects — without playing any of it, so the
 *  actual reading session is instant playback with no per-page generate-
 *  and-wait. Skips lines already cached (safe to re-run after editing
 *  text; only changed lines regenerate). Calls onProgress(done, total)
 *  as it goes. No-ops if Supabase storage isn't configured. */
export async function precacheSpreads(spreads, { voice = '', slow = false, onProgress } = {}) {
  if (!supabaseReady) return;
  const lines = (spreads || []).flatMap((s) => spreadLines(s));
